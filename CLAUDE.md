...
| `scripts/` | `setup`, `deploy`, `bump`, `package`, `decompile`, `clean` — `.ps1` for Windows, `.sh` for Linux, plus `lib.ps1` / `lib.sh`. |
...
```bash
./scripts/setup.sh       # the same six on Linux, see scripts/README.md
./scripts/deploy.sh
./scripts/bump.sh
./scripts/package.sh
