...
What each file does in full, and every rule of `../CLAUDE.md` with its reasons. `CLAUDE.md` keeps
the short form of both.
...
| `ROADMAP.md` | The plan: what is built, what is still to check, what comes next; the design behind each feature is in `docs/`. |
| `docs/` | The design behind each feature, with the game facts it rests on (see the table in `CLAUDE.md`). |
...
- **Names live in the config, not the code.** Every prefab name in the defaults exists in the
  ...
  corrected name goes into the defaults in `src/OdinsCompass.cs` and the table in `seeking.md`.
...
- Eight items, one status effect: a tier is its own prefab (`OdinsCompass1`..`8`), cloned from
  ...
  `items.md` for why the vanilla upgrade maths cannot do it.
